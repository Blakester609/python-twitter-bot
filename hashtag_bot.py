...
""" A module to fetch tweets with certain hashtags and save 
them for further analysis 

Guide: https://app.pluralsight.com/guides/building-a-twitter-bot-with-python
"""
...
def get_tweets(api, query):
    ...
    for status in tweepy.Cursor(api.search,
                                ...
                                lang="en").items():
        # Getting only tweets that have english dialects
...
def get_trending_hashtags(api, location):
    ...
    for woeid in woeids:
        try:
            ...
            trends = api.trends_place(woeid)
        # Cheking for English dialect Hashtags and storing text without '#'
...
def twitter_bot(api, locations):
    ...
    for hashtag in hashtags:
        try:
            ...
            print("API limit exceeded. Waiting for next hour")
            time.sleep(5) # change to 0.2 sec for testing
            tweets = get_tweets(api, "#"+hashtag)
        for tweet in tweets:
...
def main():
    """
    Use location = [] for getting trending tags from different countries.
